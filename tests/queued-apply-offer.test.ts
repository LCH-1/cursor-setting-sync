...
describe("the queued-apply offer", () => {
  ...
  it("says exactly what the helper drains after its initial page", () => {
    // The handoff remains bounded, but the helper now reads the durable queue
    // and advances through every later page in the same offline run. The text
    // must describe that automatic continuation rather than promise another
    // prompt/restart cycle.
    ...
    expect(truncated).toMatch(/processed automatically/i);
    expect(truncated).toMatch(/same apply/i);
    expect(truncated).not.toMatch(/offered again/i);
  });
