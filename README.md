...
- **Repair Unavailable Chats** — Automatically finds Cursor conversations whose `composerData` still references missing or unreadable message rows, locates every unavailable row in trusted synchronized history, and repairs all unambiguous conversations together—no chat or version picker. Existing valid messages and the live title/header/composerData are preserved; only unavailable referenced rows are recovered after Cursor exits, with a fresh SQLite backup and a final race check. Choose **Repair and Restart** to apply immediately.
