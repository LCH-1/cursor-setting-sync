...
The only public command is `Cursor Setting Sync: Manage`. It has exactly six top-level actions: **Show Diagnostics**; **Sync & Apply Now** for forced synchronization and queued application; **Resolve Conflicts**; **Recover Chats…** for unavailable-chat repair and opening a preserved recovery; **Restore Data…** for synchronized-version and database-backup restore; and **Repository & Devices…** for setup/reconfiguration, repository archive, peer retirement/restoration, and local disconnect. These actions enter the same bounded protocol paths; they do not create alternate synchronization formats. The full chat-recovery audit and both restore paths remain deliberately user-initiated: the audit can require a large live-database scan, while restore expresses rollback intent and can publish the selected older state for other devices to converge on.
